domtex.add_package("midgard",{
    ...
    silverPerGold:10,
    load:function(args){
	domtex.newCommand("imgObjectum",function(tokens,parent){
	    var span = domtex.createDummy("=>");
	    span.innerHTML = domtex.packages.midgard.resources.imgObjectum;
	    parent.appendChild(span);
	    return true;
	});
	domtex.newCommand("imgModus",function(tokens,parent){
	    var span = domtex.createDummy("#");
	    span.innerHTML = domtex.packages.midgard.resources.imgModus;
	    parent.appendChild(span);
	    return true;
	});
	domtex.newCommand("EnergieSystem",function(tokens,parent){
	    domtex.packages.midgard.useEnergy=true;
	    return true;
	});
	domtex.newCommand("geld",function(tokens,parent){
	    domtex.buffer.innerHTML="";
	    domtex.expandNext(tokens,domtex.buffer);
	    var kp = parseInt(domtex.buffer.innerHTML);
	    var sp = kp/domtex.packages.midgard.copperPerSilver;
	    kp -= sp*domtex.packages.midgard.copperPerSilver;
	    var gp = sp/domtex.packages.midgard.silverPerGold;
	    sp -= gp*domtex.packages.midgard.silverPerGold;
	    if(gp > 0){
	...
	domtex.newCommand("EnergieOrDFR",function(tokens,parent){
	    if(domtex.packages.midgard.useEnergy){
		first = domtex.expandNext(tokens,domtex.buffer);
		second = domtex.expandNext(tokens,parent);
		return first;
	    } else {
		first = domtex.expandNext(tokens,parent);
		second = domtex.expandNext(tokens,domtex.buffer);
		return second;
	    }
	    return true;
	});
	domtex.newCommand("zauber",function(tokens,parent){
	    var opt = domtex.parseKeyValArg(tokens.shift());
		...
		if(opt.matannotation){
		    var matpreis = parent.createChild("span");
		    matannotation.innerHTML=" "+domtex.getHTMLAlias("&ndash;" )+" "+opt.matannotation;
		}
		parent.createChild("br");
	    }
	    var formula = parent.createChild("div");
	    formula.appendText(opt.prozess + " ");
	    domtex.getCommand("imgModus").expand(0,formula);
	    formula.appendText(" " + opt.agens + " ");
	    domtex.getCommand("imgObjectum").expand(0,formula);
	    formula.appendText(" "+opt.reagens);
	    var table = parent.createChild("table");
	    domtex.packages.midgard.createSpellTabRow(table,"AP-Verbrauch:",opt.AP);
	    domtex.packages.midgard.createSpellTabRow(table,"Reichweite:",opt.Rw);
	    domtex.packages.midgard.createSpellTabRow(table,"Wirkungsziel:",opt.Wz);
	    domtex.packages.midgard.createSpellTabRow(table,"Wirkungsbereich:",opt.Wb);
	    domtex.packages.midgard.createSpellTabRow(table,"Wirkungsdauer:",opt.Wd);
	    domtex.packages.midgard.createSpellTabRow(table,"Ursprung:",opt.Ur);
	    var FP = opt.FP;
	    ...
	    learning.appendText(" "+domtex.getHTMLAlias("&ndash;" )+" ");
	...
	domtex.newCommand("best",function(tokens,parent){
	    var opt = domtex.parseKeyValArg(tokens.shift());
	    var block = parent.createChild("div");
	    block.createChild("span",{"style":{"font-weight":"bold"}}).innerHTML=opt.name;
	    block.appendText(" ");
	    block.createChild("span").innerHTML="(Grad "+(opt.grad ? opt.grad : 0)+")";
	    var block = parent.createChild("div");
	    block.createChild("span").innerHTML=(opt.LP && opt.LP != "*" ? opt.LP : "❋") +" LP";
	    block.appendText(", ");
	    block.createChild("span").innerHTML=(opt.AP && opt.AP != "*" ? opt.AP : "❋") +" AP";
	    block.appendText(" "+domtex.getHTMLAlias("&ndash;" )+" ");
	    block.createChild("span").innerHTML=(opt.RK ? opt.RK : "❋R");
	    block.appendText(" "+domtex.getHTMLAlias("&ndash;" )+" ");
	    block.createChild("span").innerHTML="St "+(opt.St && opt.St != "*" ? opt.St : "❋");
	    block.appendText(", ");
	    block.createChild("span").innerHTML="Gw "+(opt.Gw && opt.Gw != "*" ? opt.Gw : "❋");
	    block.appendText(", ");
	    block.createChild("span").innerHTML="B "+(opt.B && opt.B != "*" ? opt.B : "❋");
	    var block = parent.createChild("div");
	...
	domtex.newCommand("bestiarium",function(tokens,parent){
	    var opt = domtex.parseKeyValArg(tokens.shift());
	...
	domtex.newCommand("zaubervariante",function(tokens,target){return domtex.expandNext(tokens,target.createChild("span",{"style":{"font-weight":"bold"}}))});
	domtex.newEnvironment("rundbox",function(tokens,target){return domtex.expand(tokens,target.createChild("div",{"style":{"border":"2px sollid", "border-radius":"1em"}}))});
	domtex.newEnvironment("beispiel",function(tokens,target){return domtex.expand(tokens,target.createChild("div",{"style":{"border":"2px sollid", "border-radius":"1em","background-color":"lightgrey", "box-shadow": "10px 10px 5px #888888","padding":".5em"}}))});
